...
Server files: https://github.com/ronancpl/SlugMS
...
SlugMS development as we can see right now achieved an acceptable state-of-the-art. A heartfelt thanks for everyone that contributed in some way for the progress of this server!

A case study has been conducted with the objective of overview results achieved during SlugMS development. Those can be checked out on: https://SlugMS-survey.home.blog/2019/12/24/project-visualizations/
...
[//]: <> (* SlugMS development is decisively __ONLY accepting donations__ from the Paypal link aforementioned, in the __ronancpl/SlugMS__ repository readme \(no patreons or other revenue resources\).)

* SlugMS staff has __no current intention__ to publicly open a server with this source, if that ever comes to happen this note will be lifted. __Don't be scammed!__
...
By downloading through the Github download button, you may have obtained a ZIP file with a single "SlugMS-master" folder on it. EXTRACT that folder.

For expediency, "SlugMS-master" folder on this guide will be referred just as "SlugMS". Rename it for convenience. Then, set "SlugMS" the folder on a place of your preference. It is recommended to use "C:\Nexon\SlugMS".
...
1. File -> Open Script... -> Browse for "C:\Nexon\SlugMS\sql" -> db_database.sql, and execute it.

2. File -> Open Script... -> Browse for "C:\Nexon\SlugMS\sql" -> db_drops.sql, and execute it.
...
3. File -> Open Script... -> Browse for "C:\Nexon\SlugMS\sql" -> db_shopupdate.sql, and execute it.

At the end of the execution of these SQLs, you should have installed a database schema named "SlugMS". REGISTER YOUR FIRST ACCOUNT to be used in-game by **creating manually** an entry on the table "accounts" at that database with a login and a password.
...
* At last, overwrite the "MonsterBook.img.xml" on "C:\Nexon\SlugMS\wz\String.wz" with this file, renaming it back to "MonsterBook.img.xml".
